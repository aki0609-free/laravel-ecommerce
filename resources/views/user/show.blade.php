        ...
        <div class="p-6 bg-white border-b border-gray-200">
          <div class="md:flex md:justify-around">
            ...
            <div class="md:w-1/2 ml-4">
              <h2 class="text-sm title-font mb-4 text-gray-500 tracking-widest">{{ $product->category->name }}</h2>
              <h1 class="text-gray-900 mb-4 text-3xl title-font font-medium mb-1">{{ $product->name }}</h1>
              ...
              <div class="flex justify-around items-center">
                ...
                <form method="POST" action="{{ route('user.cart.add') }}">
                  ...
                  <button class="flex ml-auto text-white bg-indigo-500 border-0 py-2 px-6 focus:outline-none hover:bg-indigo-600 rounded">Put to Cart</button>
          ...
          <div class="mb-4 text-center">Shop selling this product</div>
          <div class="mb-4 text-center">{{ $product->shop->name }}</div>
          <div class="mb-4 text-center">
            @if ($product->shop->filename !== null)
            <img class="w-40 h-40 rounded-full mx-auto object-cover" src="{{ asset('storage/shops/'.$product->shop->filename) }}" alt="shop">
            @else
            <img src="" alt="No Image">
            @endif
          </div>
          <div class="mb-4 text-center">
            <button data-micromodal-trigger="modal-1" href='javascript:;' type="button" class="ml-auto text-white bg-indigo-500 border-0 py-2 px-6 focus:outline-none hover:bg-indigo-600 rounded">Details</button>
          </div>
        </div>
      </div>
    </div>
